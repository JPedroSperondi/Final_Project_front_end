...
document.addEventListener("DOMContentLoaded", () => {
  ...
  payBtn.addEventListener("click", async () => {
    const priceId = payBtn.dataset.price; // must be a Stripe Price ID

    try {
      // Send the priceId to your backend
      ...
      const stripe = Stripe("pk_test_51SVI74HwKGlzh6cfgcjemjdsqFWtT0TuBp43EZ3Mym7jWoGb1XOEmsURInrUjBSxKKQHM8rJOpof3NJ72mK3TtIF00e1mTQJTa");
      console.log("Server response:", session);
      await stripe.redirectToCheckout({ sessionId: session.id }); // await added for safety
